...
class File {
    ...
    public static function listDirectories($root_dir, $with_root=false, $recursive=false)
    {
        ...
        foreach ($listing as $directory) {
            ...
            if ($with_root && $recursive) {
                $subdir = File::listDirectories($full_dir,true,true);
                if ($with_root) {
                    if (!empty($subdir)) {
    ...
    public static function readDirectory($path, $directories_only=false, $files_only=false, $recursive=false, $extensions=null, $indeep=false)
    {
        ...
        while($file = $dir->read()) {
            ...
            if (is_dir($fullpath)) {
                ...
                if ($recursive) {
                    $subdir = File::readDirectory($fullpath, $directories_only, false, true, $extensions, true);
    ...
    public static function copy_directory($source_directory, $dest_directory, $overwrite=true, $hidden=false) {
        File::appendSlash($source_directory);
        File::appendSlash($dest_directory);

        if (!is_dir($dest_directory)) {
            if(!@mkdir($dest_directory)) {
                Error::log(DIR_CANT_CREATE, 'core', 'File::copy_directory', $dest_directory);
                return false;
            }
        ...
        if (!is_writable($dest_directory)) {
            Error::log(DIR_NOT_WRITABLE, 'core', 'File::copy_directory', $dest_directory);
            return false;
        }
            ...
            if (is_file($src_file)) {
                ...
                if (!@copy($src_file, $dest_file)) {
                    if ($dest_exists && !is_writable($dest_file)) {
                        Error::log(FILE_NOT_WRITABLE, 'core', 'File::copy_directory', $dest_file);
                    } else {
                        Error::log(FILE_NO_COPY, 'core', 'File::copy_directory', $dest_file);
                    }
                } else {
                    @chmod($dest_file, 0644);
                }
            }  elseif (is_dir($src_file)) {
                if(!File::copy_directory($source_directory . $file_name . '/', $dest_file . '/')) {
                    return false;
                }
            }
        }
    ...
    public function recursiveFileCopy($source_dir, $dest_dir) {
        return File::copy_directory($source_dir, $dest_dir);
    }

    public static function writeFile($filename, $text, $allowOverwrite=false, $errorReport=false)
    {
        if (!$allowOverwrite) {
    ...
    public static function readFile($filename, $error=NULL)
    {
    ...
    public static function fileCopy($file_origin, $destination_directory, $file_destination, $overwrite, $fatal)
    {
        if ($fatal) {
            ...
            if (!$fp) {
                return Error::get(FILE_CANT_READ, 'core', 'File::fileCopy', $file_origin);
            }

            $dir_check = @is_writable($destination_directory);
            if (!$dir_check) {
                return Error::get(DIR_NOT_WRITABLE, 'core', 'File::fileCopy', $destination_directory);
            }
    ...
    public function makeDir($pathname, $permissions=NULL)
    {
        ...
        if ($permissions != NULL) {
            ...
            $dirCreated = @mkdir($pathname, DIR_PERMISSIONS);
        }
    ...
    public function rotateImage($source_dir, $dest_dir, $degrees)
    {
        ...
        $source = File::_imageCopy($source_dir, $file_type);
        $rotate = imagerotate($source, $degrees, 0);

        $result = File::_writeImageCopy($rotate, $dest_dir, $file_type);
    ...
    public function cropPercent($source_dir, $dest_dir, $percentage, $origin=5)
    {
        ...
        return File::cropImage($source_dir, $dest_dir, $new_width, $new_height, $origin);
    }
    ...
    public function cropImage($source_dir, $dest_dir, $new_width, $new_height, $origin=5) {
        ...
        $source_image = File::_imageCopy($source_dir, $file_type);
        $resampled_image = File::_resampleImage($new_width, $new_height);
        ...
        $result = File::_writeImageCopy($resampled_image, $dest_dir, $file_type);
    ...
    public function resizeImage($source_dir, $dest_dir, $new_width, $new_height, $force_png=false) {
        return File::scaleImage($source_dir, $dest_dir, $new_width, $new_height);
    }
    ...
    public function makeThumbnail($fileName, $directory, $tndirectory, $maxWidth=125, $maxHeight=125, $replaceFile=false) {
        ...
        if (!File::scaleImage($source_dir, $dest_dir, $maxWidth, $maxHeight)) {
            return false;
        } else {
        ...
        if(is_dir($dir)) {
            ...
            while($file = readdir($handle)) {
                if($file == '.' || $file == '..') {
                    ...
                    if (!$result) {
                        Error::log(FILE_DELETE_DENIED, 'core', 'File::rmdir', $dir . $file);
                        return false;
                    }
                }
            }
            ...
            $result = @rmdir($dir);
            if (!$result) {
                Error::log(DIR_DELETE_DENIED, 'core', 'File::rmdir', $dir);
                return false;
            }
    ...
    {
        $all_file_type = File::getAllFileTypes();
        if (empty($ext)) {
            $ext = File::getFileExtension($file_path);
        }

        $ext = strtolower($ext);

        if (@$file_info = $all_file_type[$ext]) {
            $mime_type = File::getMimeType($file_path);
            if (function_exists('finfo_open')) {
    ...
    public function getVbType($ext)
    {
        ...
        if (strpos($ext, '.')) {
            $ext = File::getFileExtension($ext);
        }
        $all_types = File::getAllFileTypes();
