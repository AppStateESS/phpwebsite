<?php
   ...
   * Crutch form file
   *
   * Assists transition from modules written before 1.x
   * 
   * @version $Id$
...
class CrutchForm {
    ...
    function formHidden($name, $value=NULL) {
        if (is_array($name)) {
            foreach ($name as $ele_name => $ele_value) {
                $form = & new Form_Hidden($ele_name, $ele_value);
                $ele_list[] = $form->get();
            }
            return implode(chr(10), $ele_list);
        }
