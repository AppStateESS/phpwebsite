 ...
 *
 * @version $Id: DBPager.php 8373 2012-11-19 15:22:15Z matt $
...
class DBPager {
    ...
    {
        // Used to throw an Error object, now is an exception
        if (empty($table)) {
            throw new Exception('DBPager is missing a table');
        }

        /**
         * A search of the method using this variable don't show any support
        ...
        // Have to maintain original PHPWS_DB class for this because it is
        // often accessed externally ($this->db is a public variable)
        $this->db = new PHPWS_DB($table);
        $this->table = $table;

        // error statement removed here as we depend on exceptions
    ...
    /**
     * Checks the request superglobal to determine the current page number. The
     * result is saved in the object's current_page variable.
     */
    private function loadCurrentPage()
    {
        $req = Request::singleton();
        if ($req->isGetVar('change_page')) {
            $this->current_page = $req->getGet('change_page');
        } else {
            $pg = $req->isGetVar('pg') ? $req->getGet('pg') : 1;
            $this->current_page = $pg == 'last' || is_numeric($pg) ? $pg : 1;
        }
    }

    public function getError()
    {
        exit('<a href="xdebug:///' . __FILE__ . '@' . __LINE__ . '">' . __CLASS__ . '::' . __FUNCTION__ . '</a>');
        return $this->error;
    }
    ...
    public function addRowTags()
    {
        ...
        // dropped the php check that was here
    ...
    /**
     * An associative array of template tags that are added to the pager template
     * @param array $tags
     */
    public function addPageTags(array $tags)
    {
        $this->page_tags = $tags;
    }
    ...
    public function getLimit()
    {
        if (empty($this->limit) || !in_array($this->limit, $this->limitList)) {
            $this->limit = current($this->limitList);
    ...
    /**
     * Returns the count of total rows the pager can show
     * @return integer
     */
    ...
    /**
     * Returns a set of links allowing the user to change the amount of item shown
     * per page
     * @return string
     */
    ...
    /**
     * Runs self::initialize() and returns the result of the found rows. Output is
    ...
    /**
     * Unsets the DB_Cache session variable.
     */
