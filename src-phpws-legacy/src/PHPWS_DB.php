...
class PHPWS_DB
{
    ...
    public $group_in_groups = array();
    public $group_in_list = array();
    // This variable holds a sql query string
    public $sql = null;
    protected $_allColumns = null;
    protected $_columnInfo = null;
    protected $_lock = false;
    // contains the database specific factory class
    protected $_distinct = false;
    protected $_test_mode = false;
    protected $_join = null;
    protected $_join_tables = null;
    ...
    {
        if (!\phpws\PHPWS_DB::allowed($table)) {
            return false;
        }
    ...
    public function getTableColumns($fullInfo = false)
    {
        ...
        foreach ($this->tables as $table) {
            ...
            if (\phpws\PHPWS_Error::isError($columns)) {
                \phpws\PHPWS_Error::log('Could not get columns in table: ' . $table);
                \phpws\PHPWS_Error::log($columns);
                return $columns;
            }
    ...
    public function addTable($table, $as = null)
    {
        ...
        if (\phpws\PHPWS_DB::allowed($table)) {
            ...
            return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_TABLE_NAME, 'core', '\phpws\PHPWS_DB::addTable', $table);
        }
        return true;
    }
    ...
    public function getTable($format = true)
    {
        if (empty($this->tables)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::getTable');
        }
    ...
    {
        if (\phpws\PHPWS_DB::allowed($group_by)) {
            if (!strpos($group_by, '.')) {
    ...
    public function groupIn($sub, $main)
    {
        ...
        if (isset($this->group_in_list[$sub])) {
            $sub_group = $this->group_in_list[$sub];
        } else {
            $sub_group = new \phpws\DB\DB_Group_In;
            $sub_group->name = $sub;
            $sub_group->db = $this;
        }

        if (isset($this->group_in_list[$main])) {
            $main_group = $this->group_in_list[$main];
        } else {
            $main_group = new \phpws\DB\DB_Group_In;
            $main_group->name = $main;
            $main_group->db = $this;
        }

        $sub_group->parent = $main;
        $main_group->children[$sub] = $sub_group;

        $this->group_in_list[$main] = $main_group;
        $this->group_in_list[$sub] = $sub_group;

        $this->group_in[$main] = $main_group;
        $this->group_in_groups[$main] = 1;
        $this->group_in_groups[$sub] = 1;
        return true;
    }
    ...
    public function addWhere($column, $value = null, $operator = null, $conj = null, $group = null, $join = false)
    {
        ...
        if (is_array($column)) {
            ...
            if (!\phpws\PHPWS_DB::allowed($column) || preg_match('[^\w\.]', $column)) {
                return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::addWhere', $column);
            }
        }
        ...
        if (is_string($column)) {
            if (substr_count($column, '.') == 1) {
                ...
                if (isset($this->table_as[$join_table])) {
                    ...
                    $column = & $join_column;
                } elseif (\phpws\PHPWS_DB::inDatabase($join_table, $join_column)) {
                    $column = & $join_column;
                    $source_table = $join_table;
    ...
    public function getWhere($dbReady = false)
    {
        ...
        if ($dbReady) {
            ...
            if (!empty($where_list)) {
                $sql[] = $this->buildGroup($where_list);
            }
     ...
     */
    public function buildGroup($where_list)
    {
        $first = true;
        foreach ($where_list as $group_name => $info) {
            if (isset($this->group_in_groups[$group_name])) {
                continue;
            }
            ...
            $sql[] = '(' . implode(' ', $info['group_sql']) . ')';
        }
        if (!empty($this->group_in)) {
            foreach ($this->group_in as $group_in) {
                if ($group_in->parent) {
                    continue;
                }
    ...
    public function addColumn($column, $max_min = null, $as = null, $count = false, $distinct = false, $coalesce = null)
    {
        ...
        if (!empty($as)) {
            if (!\phpws\PHPWS_DB::allowed($as)) {
                return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::addColumn', $as);
            }
        }

        if (!\phpws\PHPWS_DB::allowed($column)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::addColumn', $column);
        }
    ...
    public function addValue($column, $value = null)
    {
        if (is_array($column)) {
            ...
            if (!\phpws\PHPWS_DB::allowed($column)) {
                return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::addValue', $column);
            }
    ...
    public function insert($auto_index = true)
    {
        ...
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::insert');
        }

        $values = $this->getAllValues();

        if (!isset($values)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_NO_VALUES, 'core', '\phpws\PHPWS_DB::insert');
        }
        ...
        foreach ($values as $index => $entry) {
            $columns[] = $index;
            $set[] = \phpws\PHPWS_DB::dbReady($entry);
        }
    ...
    public function update($return_affected = false)
    {
        ...
        if (empty($values)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_NO_VALUES, 'core', '\phpws\PHPWS_DB::update');
        }

        foreach ($values as $index => $data) {
            $columns[] = $index . ' = ' . \phpws\PHPWS_DB::dbReady($data);
        }
    ...
    public function getSelectSQL($type)
    {
        ...
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::select');
        }
    ...
    public function select($type = null, $sql = null)
    {
        ...
        if (!isset($sql)) {
            ...
            if (\phpws\PHPWS_Error::isError($sql_array)) {
                return $sql_array;
            }
        ...
        switch ($type) {
            case 'assoc':
                ...
                if (count($this->columns) == 2) {
                    ...
                    return \phpws\PHPWS_DB::_indexBy($result, $this->columns[0]['name'], true);
                }
            ...
            case 'col':
                if (empty($sql) && empty($this->columns)) {
                    return \phpws\PHPWS_Error::get(PHPWS_DB_NO_COLUMN_SET, 'core', '\phpws\PHPWS_DB::select');
                }
                ...
                if (isset($indexby)) {
                    ...
                    return \phpws\PHPWS_DB::_indexBy($result, $indexby, true);
                }
                \phpws\PHPWS_DB::logDB($sql);
        ...
        foreach ($sql as $item) {
            ...
            if ($colMode) {
                ...
                if (isset($item[$col_check]) || $item[$col_check] === null) {
                    \phpws\PHPWS_DB::_expandIndex($rows, $value, $item[$col_check], $stacked);
                }
            } else {
                \phpws\PHPWS_DB::_expandIndex($rows, $item[$indexby], $item, $stacked);
            }
        }
    ...
    public function incrementColumn($column_name, $amount = 1)
    {
        ...
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::incrementColumn');
        }
    ...
    public function delete($return_affected = false)
    {
        ...
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::delete');
        }
    ...
    public static function dropTable($table, $check_existence = true, $sequence_table = true)
    {
        ...
        if ($check_existence && !\phpws\PHPWS_DB::isTable($table)) {
            return true;
        }

        $result = \phpws\PHPWS_DB::query("DROP TABLE $table");

        if (\phpws\PHPWS_Error::isError($result)) {
            return $result;
        }

        if ($sequence_table && \phpws\PHPWS_DB::isSequence($table)) {
    ...
    public function truncateTable()
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::truncateTable()');
        }

        $sql = "TRUNCATE TABLE $table";

        return \phpws\PHPWS_DB::query($sql);
    }
    ...
    public function dropTableIndex($name = null)
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::dropTableIndex');
        }
    ...
    public function createTableIndex($column, $name = null, $unique = false)
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::createTableIndex');
        }

        if (is_array($column)) {
            foreach ($column as $col) {
                if (!$this->isTableColumn($col)) {
                    return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::createTableIndex');
                }
            }
    ...
    public function createTable()
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::createTable');
        }
    ...
    public function renameTableColumn($old_name, $new_name)
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::renameTableColumn');
        }

        $specs = $this->getColumnInfo($old_name, true);
        if (empty($specs)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::renameTableColumn', $old_name);
        }
    ...
    public function addTableColumn($column, $parameter, $after = null, $indexed = false)
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::addTableColumn');
        }

        if (!\phpws\PHPWS_DB::allowed($column)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::addTableColumn', $column);
        }
        ...
        foreach ($sql as $val) {
            $result = \phpws\PHPWS_DB::query($val);
            if (\phpws\PHPWS_Error::isError($result)) {
                return $result;
            }
        }
        ...
        if ($indexed == true) {
            ...
            $result = \phpws\PHPWS_DB::query($indexSql);
            if (\phpws\PHPWS_Error::isError($result)) {
                return $result;
            }
        }
    ...
    public function alterColumnType($column, $parameter)
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::alterColumnType');
        }

        if (!\phpws\PHPWS_DB::allowed($column)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::alterColumnType', $column);
        }
    ...
    public function dropTableColumn($column)
    {
        $table = $this->getTable(false);
        if (!$table) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_ERROR_TABLE, 'core', '\phpws\PHPWS_DB::dropColumn');
        }

        if (!\phpws\PHPWS_DB::allowed($column)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_BAD_COL_NAME, 'core', '\phpws\PHPWS_DB::dropTableColumn', $column);
        }
    ...
    {
        if (!is_file($filename)) {
            return \phpws\PHPWS_Error::get(PHPWS_FILE_NOT_FOUND, 'core', '\phpws\PHPWS_DB::importFile');
        }
        $data = file_get_contents($filename);
        return \phpws\PHPWS_DB::import($data, $report_errors);
    }
    ...
    public static function import($text, $report_errors = true)
    {
        ...
        if (!$first_import) {
            if ($report_errors) {
                return \phpws\PHPWS_Error::get(PHPWS_DB_IMPORT_FAILED, 'core', '\phpws\PHPWS_DB::import');
            } else {
                \phpws\PHPWS_Error::log(PHPWS_DB_IMPORT_FAILED, 'core', '\phpws\PHPWS_DB::import');
                $error = true;
            }
        }
    ...
    public static function homogenize(&$query)
    {
        ...
        if (\phpws\PHPWS_DB::getDBType() != 'mysql' && \phpws\PHPWS_DB::getDBType() != 'mysqli') {
            ...
            if ($rows = $this->select()) {
                ...
                foreach ($rows as $dataRow) {
                    foreach ($dataRow as $key => $value) {
                        $allKeys[] = $key;
                        $allValues[] = \phpws\PHPWS_DB::escape($value);
                    }
...
// END FUNC extractTableName
    ...
    public function dbReady($value = null)
    {
        if (is_array($value) || is_object($value)) {
            return \phpws\PHPWS_DB::dbReady(serialize($value));
...
// END FUNC dbReady()
    ...
    {
        if (!is_object($object)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_NOT_OBJECT, 'core', '\phpws\PHPWS_DB::loadObject');
        }

        if ($require_where && empty($object->id) && empty($this->where)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_NO_ID, 'core', '\phpws\PHPWS_DB::loadObject', get_class($object));
        }
        ...
        if (\phpws\PHPWS_Error::isError($variables)) {
            return $variables;
...
// END FUNC loadObject
    ...
    public function getObjects($class_name)
    {
        ...
        if (\phpws\PHPWS_Error::isError($result) || !isset($result)) {
            return $result;
        }
        ...
        if (!class_exists($class_name)) {
            return \phpws\PHPWS_Error::get(PHPWS_CLASS_NOT_EXIST, 'core', '\phpws\PHPWS_DB::getObjects', $class_name);
        }
    ...
    public function saveObject($object, $stripChar = false, $autodetect_id = true)
    {
        ...
        if (!is_array($object_vars)) {
            return \phpws\PHPWS_Error::get(PHPWS_DB_NO_OBJ_VARS, 'core', '\phpws\PHPWS_DB::saveObject');
        }
        ...
        foreach ($object_vars as $column => $value) {
            ...
            if (\phpws\PHPWS_Error::isError($isTblColumn)) {
    ...
    public static function allowed($value)
    {
        ...
        $reserved = array('ADD', 'ALL', 'ALTER', 'ANALYZE', 'AND', 'AS', 'ASC', 'AUTO_INCREMENT', 'BDB',
            ...
            'SMALLINT', 'SONAME', 'SPATIAL', 'SPECIFIC', 'SQL_BIG_RESULT', 'SQL_CALC_FOUND_ROWS', 'SQL_SMALL_RESULT',
    ...
    {
        if (!\phpws\PHPWS_DB::allowed($name)) {
            return false;
        }
    ...
    public function updateSequenceTable()
    {
        ...
        if (\phpws\PHPWS_Error::isError($max_id)) {
            return $max_id;
        }
    ...
    public static function begin()
    {
        ...
        return \phpws\PHPWS_DB::query('BEGIN');
    }
    ...
    public static function commit()
    {
        ...
        return \phpws\PHPWS_DB::query('COMMIT');
    }
    ...
    public static function rollback()
    {
        ...
        $GLOBALS['DB_Transaction'] = false;
        return \phpws\PHPWS_DB::query('ROLLBACK');
    }
    ...
    public function moveRow($order_column, $id_column, $id, $direction = 1)
    {
        ...
        if ($current_order == 1 && $direction == -1) {
            ...
            \phpws\PHPWS_DB::begin();

            if (\phpws\PHPWS_Error::logIfError($this->reduceColumn($order_column))) {
                \phpws\PHPWS_DB::rollback();
                return false;
            }
            $db->reset();
            $db->addWhere($id_column, $id);
            $db->addValue($order_column, $total_rows);
            if (\phpws\PHPWS_Error::logIfError($db->update())) {
                \phpws\PHPWS_DB::rollback();
                return false;
            }
            \phpws\PHPWS_DB::commit();
            unset($db);
            return true;
            ...
            \phpws\PHPWS_DB::begin();
            if (\phpws\PHPWS_Error::logIfError($this->incrementColumn($order_column))) {
                \phpws\PHPWS_DB::rollback();
                return false;
            }
            $db->reset();
