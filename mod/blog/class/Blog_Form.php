...
class Blog_Form {
    ...
    public static function edit(Blog $blog, $version_id = NULL, $limited = false)
    {
        ...
        $link_choices['none'] = dgettext('blog',
                'No link and ignore image link setting');
        $link_choices['default'] = dgettext('blog',
                'No link but allow image link setting');
        $link_choices['readmore'] = dgettext('blog', 'Link to read more');
        $link_choices['parent'] = dgettext('blog',
                'Link resized image to parent');
        ...
        $form->setLabel('image_link',
                dgettext('blog', 'Link setting (if image)'));
        ...
        $form->setLabel('thumbnail',
                dgettext('blog', 'Show image or media thumbnail in list view'));
        ...
        $template['REMINDER'] = dgettext('blog',
                'Add a horizontal rule to separate content into summary and body');
    ...
    public static function settings()
    {
        ...
        $form->addText('past_entries',
                PHPWS_Settings::get('blog', 'past_entries'));
        $form->setLabel('past_entries',
                dgettext('blog', 'Number of past entries'));
        ...
        $form->setLabel('show_posted_date',
                dgettext('blog', 'Show the date the post was made'));
        $form->setMatch('show_posted_date',
                PHPWS_Settings::get('blog', 'show_posted_date'));

        // Show/hide posted by user full name
        $form->addCheck('show_posted_by', 1);
        $form->setLabel('show_posted_by',
                dgettext('blog', 'Show the author\'s name'));
        $form->setMatch('show_posted_by',
                PHPWS_Settings::get('blog', 'show_posted_by'));

        $form->addCheck('simple_image', 1);
        $form->setLabel('simple_image', dgettext('blog', 'Use Image Manager'));
        $form->setMatch('simple_image',
                PHPWS_Settings::get('blog', 'simple_image'));

        $form->addCheck('mod_folders_only', 1);
        $form->setLabel('mod_folders_only',
                dgettext('blog', 'Hide general image folders'));
        $form->setMatch('mod_folders_only',
                PHPWS_Settings::get('blog', 'mod_folders_only'));

        $form->addCheck('home_page_display', 1);
        $form->setLabel('home_page_display',
                dgettext('blog', 'Show blog on home page'));
        $form->setMatch('home_page_display',
                PHPWS_Settings::get('blog', 'home_page_display'));

        $form->addCheck('logged_users_only', 1);
        $form->setLabel('logged_users_only',
                dgettext('blog', 'Logged user view only'));
        $form->setMatch('logged_users_only',
                PHPWS_Settings::get('blog', 'logged_users_only'));
        ...
        if (!empty($groups)) {
            ...
            $form->setLabel('view_only',
                    dgettext('blog', 'Limit blog to specific groups'));
        ...
        $form->setMatch('show_recent',
                PHPWS_Settings::get('blog', 'show_recent'));
        $form->addCssClass('show_recent', 'form-control');

        $form->addTextField('max_width',
                PHPWS_Settings::get('blog', 'max_width'));
        $form->setLabel('max_width',
                dgettext('blog', 'Maximum image width (50-2048)'));
        $form->setSize('max_width', 4, 4);
        $form->addCssClass('max_width', 'form-control');

        $form->addTextField('max_height',
                PHPWS_Settings::get('blog', 'max_height'));
        $form->setLabel('max_height',
                dgettext('blog', 'Maximum image height (50-2048)'));
        $form->setSize('max_height', 4, 4);
        $form->addCssClass('max_height', 'form-control');

        $form->addTextArea('comment_script',
                PHPWS_Settings::get('blog', 'comment_script'));
        $form->setLabel('comment_script',
                dgettext('blog',
                        'Paste in your comment code here (e.g. Disqus, Livefyre, Facebook, etc.)'));
        ...
        if (Current_User::isDeity()) {
            ...
            $form->setLabel('purge_date',
                    dgettext('blog', 'Purge all entries before this date'));
        ...
        if (PHPWS_Settings::get('blog', 'allow_anonymous_submits')) {
            $template['MENU_LINK'] = PHPWS_Text::secureLink(dgettext('blog',
                                    'Clip for menu'), 'blog',
        ...
        $template['PAST_NOTE'] = dgettext('blog',
                'Set to zero to prevent display');
