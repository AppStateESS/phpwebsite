...
class Menu_Admin
{
    ...
    private function menuOptions($request)
    {
        ...
        $options[] = '<option value="" selected disabled>Move link to menu below...</option>';
    ...
    private function menuList()
    {
        ...
        $vars['delete'] = 'Delete';
        $vars['confirm_delete'] = 'Confirm deletion';
        $vars['first_menu_id'] = $first_menu_id;
        $vars['authkey'] = \Current_User::getAuthKey();
        $vars['blank_title'] = 'Title must not be blank';
        $vars['title_error'] = 'Make sure you have filled in the required inputs.';
        $vars['url_error'] = 'Please enter a url or choose a PageSmith page.';
...
EOF;
        ...
        $menu_tpls[] = '<optgroup label="Menu module templates">';
        ...
        if (!empty($theme_menu_templates)) {
            $menu_tpls[] = '<optgroup label="Theme templates">';
