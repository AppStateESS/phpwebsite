    ...
    function edit()
    {
        ...
        if (PHAT_SHOW_INSTRUCTIONS) {
            $GLOBALS['CNT_phatform']['title'] = dgettext('phatform',
                    'Multiselect Element Instructions');
        }
        ...
        $editTags['SIZE_INPUT'] = PHPWS_Form::formTextField('PHAT_ElementSize',
                        $this->_size, 5, 3);
        ...
        $elements[0] .= PHPWS_Template::processTemplate($editTags, 'phatform',
                        'multiselect/edit.tpl');

        return PHPWS_Form::makeForm('PHAT_MultiselectEdit', 'index.php',
...
// END FUNC edit
    ...
    function save()
    {
        ...
        if ((!$_SESSION['PHAT_FormManager']->form->checkLabel($_REQUEST['PHAT_ElementName']) && (strcasecmp($label,
            ...
            $message = dgettext('phatform',
                    'The name you entered for the Multiselect is not valid or is already in use with this form.');
            $currentError = PHPWS_Error::get(PHATFORM_INVALID_NAME, 'phatform',
                            'PHAT_Multiselect::save()');
            $error = TRUE;
        }
