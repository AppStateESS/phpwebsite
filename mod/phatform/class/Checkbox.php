 ...
 * Checkbox item
 *
 * @version $Id: Checkbox.php 20 2006-10-18 18:23:18Z matt $
 * @author  Steven Levin
 * @author  Adam Morton
...
class PHAT_Checkbox extends PHAT_Element {
    ...
    function edit() {
        ...
        if(PHAT_SHOW_INSTRUCTIONS) {
            $GLOBALS['CNT_phatform']['title'] = _('Checkbox Element Instructions');
        }
    ...
    function save() {
        ...
        if((!$_SESSION['PHAT_FormManager']->form->checkLabel($_REQUEST['PHAT_ElementName']) && (strcasecmp($label, $_REQUEST['PHAT_ElementName']) != 0))
           || PEAR::isError($this->setLabel(PHPWS_DB::sqlFriendlyName($_REQUEST['PHAT_ElementName'])))) {
            ...
            $message = _('The name you entered for the Checkbox is not valid or is already in use with this form.');
