...
class WikiManager
{
    function sendMessage($message, $command, $secure=TRUE)
    {
        ...
        if (is_array($command))
        {
            ...
            {
                PHPWS_Core::reroute('wiki/' . $command['page']);
            }
            else
            {
                PHPWS_Core::reroute(PHPWS_Text::linkAddress('wiki', $command, $secure));
            }
        }
        else
        {
            PHPWS_Core::reroute(PHPWS_Text::linkAddress('wiki', array('op'=>$command), $secure));
        }
    }
    ...
    function sendEmail()
    {
        if(PHPWS_Settings::get('wiki', 'monitor_edits'))
        {
            ...
            $message = str_replace('[url]', PHPWS_Core::getHomeHttp() .
