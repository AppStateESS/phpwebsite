...
class Calendar_Event {
    ...
    public function getSummary($linked = true)
    {
        if ($linked) {
            ...
            return sprintf('<a href="%s" class="url">%s</a>', $url,
                    $this->summary);
        } else {
            return $this->summary;
        }
    ...
    public function tplFormatTime()
    {
        ...
        if ($this->all_day) {
            ...
            if (date('Ymd', $this->start_time) != date('Ymd', $this->end_time)) {
                ...
                if (CALENDAR_MONTH_FIRST) {
                    if (date('Y', $this->start_time) != date('Y',
                                    $this->end_time)) {
                        $tpl['START_TIME'] = sprintf(dgettext('calendar',
                                        'All day event, %s'),
                                strftime('%B %e, %Y', $this->start_time));
                    } else {
                        $tpl['START_TIME'] = sprintf(dgettext('calendar',
                                        'All day event, %s'),
                    ...
                    if (date('Y', $this->start_time) != date('Y',
                        ...
                        $tpl['START_TIME'] = sprintf(dgettext('calendar',
                                        'All day event, %s'),
                                strftime('%e, %Y', $this->start_time));
                    } else {
                        $tpl['START_TIME'] = sprintf(dgettext('calendar',
                                        'All day event, %s'),
                                strftime('%e', $this->start_time));
                    }
                }
            ...
            $tpl['END_TIME_HOUR'] = $this->getEndTime(CALENDAR_TIME_FORMAT);
            $tpl['END_TIME_MONTH'] = $this->getEndTime($month_day_mode);
            $tpl['END_TIME_YEAR'] = $this->getEndTime('%Y');
    ...
    public function init()
    {
        ...
        if (empty($table)) {
            return PHPWS_Error::get(CAL_EVENT_TABLE_MISSING, 'calendar',
                            'Calendar::init', $table);
        }
    ...
    public function post($suggested = false)
    {
        ...
        if ($this->all_day) {
            $startTime = mktime(0, 0, 0, date('m', $start_date),
                    date('d', $start_date), date('Y', $start_date));
            $endTime = mktime(23, 59, 59, date('m', $end_date),
                    (int) date('d', $end_date), date('Y', $end_date));
        } else {
            $startTime = mktime($start_time_hour, $start_time_minute, 0,
                    date('m', $start_date), date('d', $start_date),
                    date('Y', $start_date));
            $endTime = mktime($end_time_hour, $end_time_minute, 0,
                    date('m', $end_date), date('d', $end_date),
                    date('Y', $end_date));
        }
        ...
        if (!$suggested && isset($_POST['repeat_event'])) {
            ...
            if (date('Ymd', $this->end_repeat) <= date('Ymd', $this->start_time)) {
                $errors[] = dgettext('calendar',
                        'The date to repeat until must be greater than the event\'s start date.');
            }
                ...
                switch ($_POST['repeat_mode']) {
                    ...
                    case 'monthly':
                        ...
                        if (empty($_POST['monthly_repeat'])) {
                            $errors[] = dgettext('calendar',
                                    'Please pick a monthly repeat method.');
                            break;
                        }
                        switch ($_POST['monthly_repeat']) {
                            ...
                            default:
                                $errors[] = dgettext('calendar',
                                        'Please pick a proper monthly repeat method.');
                        }
                        break;
                    ...
                    case 'every':
                        if (empty($_POST['every_repeat_number']) ||
                            ...
                            $errors[] = dgettext('calendar',
                                    'Please choose options for the "Every" repeat method.');
                            break;
                        }
    ...
    public function icalTags()
    {
        ...
        $description = preg_replace(array('/^\s\s+/', '/\s\s+$/', '/\s\s+/u'),
                array('', '', ' '), $description);
        ...
        if ($this->all_day) {
            $tpl['DTSTART_AD'] = PHPWS_Time::getDTTime($this->start_time,
                            'all_day');
