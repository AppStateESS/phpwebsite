...
class Podcaster_Forms {
    ...
    {
        $form = new PHPWS_Form('podcaster_channel');
        ...
        $manager->maxImageHeight(PHPWS_Settings::get('podcaster', 'max_height'));
        ...
        $this->podcaster->content = PHPWS_Template::process($tpl, 'podcaster', 'edit_channel.tpl');
    }
    ...
    function editEpisode()
    {
        ...
        $this->podcaster->content = PHPWS_Template::process($tpl, 'podcaster', 'edit_episode.tpl');
    }
    ...
    function listChannels()
    {
        ...
        if ($num == '0') {
            if (Current_User::allow('podcaster', 'edit_channel')) {
                ...
                $ptags['EMPTY_MESSAGE'] = sprintf(dgettext('podcaster', 'Check your %s then create a %s to begin'), PHPWS_Text::secureLink(dgettext('podcaster', 'Settings'), 'podcaster', $vars),  PHPWS_Text::secureLink(dgettext('podcaster', 'New Channel'), 'podcaster', $vars2));
            } else {
    ...
    function editSettings()
    {
        ...
        $form = new PHPWS_Form('podcaster_settings');
        $form->addHidden('module', 'podcaster');
        $form->addHidden('aop', 'post_settings');

        $form->addText('channel_limit', PHPWS_Settings::get('podcaster', 'channel_limit'));
        ...
        $form->addText('cache_timeout', PHPWS_Settings::get('podcaster', 'cache_timeout'));
        ...
        $form->setMatch('show_block', PHPWS_Settings::get('podcaster', 'show_block'));
        ...
        $this->podcaster->content = PHPWS_Template::process($tpl, 'podcaster', 'edit_settings.tpl');
    }
    ...
    function showInfo()
    {
        ...
        $this->podcaster->content = PHPWS_Template::process($tpl, 'podcaster', 'info.tpl');
    }
