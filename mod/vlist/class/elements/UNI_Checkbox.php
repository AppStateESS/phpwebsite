...
class UNI_Checkbox extends UNI_Element {
    ...
    function view($match=null) 
    {
        ...
        for($i = 0; $i < sizeof($optionId); $i++) {
            $tpl['CHECK_BOXES'] .= PHPWS_Form::formCheckBox('UNI_' . $this->id . '[' . $i . ']', $optionId[$i], $match, null, $optionText[$i]) . "<br />\n";
        }

        return PHPWS_Template::processTemplate($tpl, 'vlist', 'elements/view_checkbox.tpl');
    } 
    ...
    function edit() 
    {
        ...
        $elements[0] .= PHPWS_Template::processTemplate($tpl, 'vlist', 'elements/edit_checkbox.tpl');

        return PHPWS_Form::makeForm('UNI_Checkbox_edit', 'index.php', $elements, 'post', null, null);
    } 
