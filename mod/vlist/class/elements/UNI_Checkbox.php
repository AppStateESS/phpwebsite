...
class UNI_Checkbox extends UNI_Element {
    ...
    function view($match=null)
    {
        ...
        for($i = 0; $i < sizeof($optionId); $i++) {
            $tpl['CHECK_BOXES'] .= Core\Form::formCheckBox('UNI_' . $this->id . '[' . $i . ']', $optionId[$i], $match, null, $optionText[$i]) . "<br />\n";
        }

        return Core\Template::processTemplate($tpl, 'vlist', 'elements/view_checkbox.tpl');
    }
    ...
    function edit()
    {
        ...
        $elements[0] =  Core\Form::formHidden('module', 'vlist') . 
                        ...
                        Core\Form::formHidden('type', 'Checkbox') ;
        if ($this->id) {
        ...
        $elements[0] .= Core\Template::processTemplate($tpl, 'vlist', 'elements/edit_checkbox.tpl');

        return Core\Form::makeForm('UNI_Checkbox_edit', 'index.php', $elements, 'post', null, null);
    }
