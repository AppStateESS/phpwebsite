...
//            case 'delete_attribute':
            ...
            case 'menu':
                if (isset($_GET['tab'])) {
                    PHPWS_Core::initModClass('vshop', 'vShop_Forms.php');
                    if ($_GET['tab'] == 'settings' || $_GET['tab'] == 'taxes' || $_GET['tab'] == 'option_sets' || $_GET['tab'] == 'option_values') {
                        $settingsPanel = vShop_Forms::settingsPanel();
                        $settingsPanel->enableSecure();
...
// old                        PHPWS_Core::reroute('index.php?module=vshop&dept='.$this->item->dept_id);
            ...
            case 'view_option_set':
                $settingsPanel->setCurrentTab('option_sets');
                $this->loadOption_set();
                $this->title = $this->option_set->getTitle(true);
                $this->content = $this->option_set->view();
                break;
            ...
            case 'post_option_set':
                ...
                if ($this->postOption_set()) {
                    if (PHPWS_Error::logIfError($this->option_set->save())) {
                        $this->forwardMessage(dgettext('vshop', 'Error occurred when saving option set.'));
                        PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=option_sets');
                    } else {
                        $this->forwardMessage(dgettext('vshop', 'Option set saved successfully.'));
                        PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=option_sets');
                    }
                } else {
                    $this->loadForm('edit_option_set');
            ...
            case 'delete_option_set':
                ...
                $this->loadOption_set();
                $this->option_set->delete();
                $this->message = dgettext('vshop', 'Option set deleted.');
                $this->loadForm('option_sets');
                break;
            ...
            case 'view_option_value':
                ...
                $this->title = $this->option_value->getTitle(true);
                $this->content = $this->option_value->view();
                break;
...
//                $this->loadOption_value();
                $this->loadForm('edit_option_value');
                break;
            ...
            case 'post_option_value':
                ...
                if ($this->postOption_value()) {
                    if (PHPWS_Error::logIfError($this->option_value->save())) {
                        $this->forwardMessage(dgettext('vshop', 'Error occurred when saving option set value.'));
                        PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=option_values');
                    } else {
                        $this->forwardMessage(dgettext('vshop', 'Option set value saved successfully.'));
                        PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=option_values');
                    }
                } else {
                    $this->loadForm('edit_option_value');
            ...
            case 'delete_option_value':
                ...
                $this->loadOption_value();
                $this->option_value->delete();
                $this->message = dgettext('vshop', 'Option set value deleted.');
                $this->loadForm('option_values');
                break;
...
//                $settingsPanel->setCurrentTab('option_values');
                $this->loadAttribute();
                $this->title = $this->attribute->getTitle(true);
                $this->content = $this->attribute->view();
                break;
...
//                $settingsPanel->setCurrentTab('option_values');
                ...
                if ($this->postAttribute()) {
                    if (PHPWS_Error::logIfError($this->attribute->save())) {
                        $this->forwardMessage(dgettext('vshop', 'Error occurred when saving attribute.'));
                        PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=option_values');  // CHECK !!!
                    } else {
                        $this->forwardMessage(dgettext('vshop', 'Attribute saved successfully.'));
                        PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=option_values');  // CHECK !!!
                    }
                } else {
                    $this->loadForm('edit_attribute');
...
//                $settingsPanel->setCurrentTab('option_values');
                ...
                $this->message = dgettext('vshop', 'Attribute deleted.');
                $this->loadForm('attributes'); // CHECK !!!
                break;
...
//                $settingsPanel->setCurrentTab('orders');
            ...
            case 'delete_order':
                ...
                $this->loadOrder();
                $this->order->delete();
                $this->forwardMessage = dgettext('vshop', 'Order deleted.');
                PHPWS_Core::reroute('index.php?module=vshop&aop=menu&tab=' . $_REQUEST['tab']);
                break;
...
// old                if (PHPWS_Settings::get('vshop', 'use_inventory') && $new_qty <= $this->item->stock) {
                if ((PHPWS_Settings::get('vshop', 'use_inventory') && $new_qty <= $this->item->stock) || (!PHPWS_Settings::get('vshop', 'use_inventory'))) { // fixed thanks wendall
                    print_r($_REQUEST); exit; // TEMP
                    /* need to test for options here and if there are, build data array for them */
                    if (isset($_REQUEST['options'])) {
//                        $options = $_REQUEST['options'];
                        PHPWS_Core::initModClass('vshop', 'vShop_Option_set.php');
                        PHPWS_Core::initModClass('vshop', 'vShop_Attribute.php');
                        /* loop through the options */
                        foreach ($_REQUEST['options'] as $o_set) {
                            /* if the option is an array - checkboxes */
                            if (is_array($o_set)) {
                            } else {
                            }
                        }
                    ...
                    $cart->addItems($this->item->id, $options, $qty);
...
//print_r($this->forms->vshop); exit;
    ...
    public function loadOption_set($id=0)
    {
        PHPWS_Core::initModClass('vshop', 'vShop_Option_set.php');

        if ($id) {
            $this->option_set = new vShop_Option_set($id);
        } elseif (isset($_REQUEST['option_set_id'])) {
            $this->option_set = new vShop_Option_set($_REQUEST['option_set_id']);
//        } elseif (isset($_REQUEST['id'])) {
//            $this->option_set = new vShop_Option_set($_REQUEST['id']);
        } elseif (isset($_REQUEST['option_set'])) {
            $this->option_set = new vShop_Option_set($_REQUEST['option_set']);
        } else {
            $this->option_set = new vShop_Option_set;
        }
    }


    public function loadOption_value($id=0)
    {
        PHPWS_Core::initModClass('vshop', 'vShop_Option_value.php');

        if ($id) {
            $this->option_value = new vShop_Option_value($id);
        } elseif (isset($_REQUEST['option_value_id'])) {
            $this->option_value = new vShop_Option_value($_REQUEST['option_value_id']);
//        } elseif (isset($_REQUEST['id'])) {
//            $this->option_value = new vShop_Option_value($_REQUEST['id']);
        } elseif (isset($_REQUEST['option_value'])) {
            $this->option_value = new vShop_Option_value($_REQUEST['option_value']);
        } else {
            $this->option_value = new vShop_Option_value;
        }
//print_r($this->option_value->set_id); exit;
//print_r($this->option_set); exit;
        if (empty($this->option_set)) {
            if (isset($this->option_value->set_id)) {
                $this->loadOption_set($this->option_value->set_id);
            } elseif (isset($_REQUEST['option_set_id'])) {
                $this->loadOption_set($_REQUEST['option_set_id']);
                $this->option_value->set_id = $_REQUEST['option_set_id'];
            } else {
                $this->loadOption_set();
                $this->option_value->set_id = $this->option_set->id;
            }
        }
    }


    public function loadAttribute($id=0)
    {
        PHPWS_Core::initModClass('vshop', 'vShop_Attribute.php');

        if ($id) {
            $this->attribute = new vShop_Attribute($id);
        } elseif (isset($_REQUEST['attribute_id'])) {
            $this->attribute = new vShop_Attribute($_REQUEST['attribute_id']);
//        } elseif (isset($_REQUEST['id'])) {
//            $this->attribute = new vShop_Attribute($_REQUEST['id']);
        } elseif (isset($_REQUEST['attribute'])) {
            $this->attribute = new vShop_Attribute($_REQUEST['attribute']);
        } else {
            $this->attribute = new vShop_Attribute;
        }

        if (empty($this->item)) {
            if (isset($this->attribute->item_id)) {
                $this->loadItem($this->attribute->item_id);
            } elseif (isset($_REQUEST['item_id'])) {
                $this->loadItem($_REQUEST['item_id']);
                $this->attribute->item_id = $_REQUEST['item_id'];
            } else {
                $this->loadItem();
                $this->attribute->item_id = $this->item->id;
            }
        }
    }
...
// old        if (!empty($_POST['shipping'])) {
    ...
    public function postOption_set()
    {
        $this->loadOption_set();

        if (empty($_POST['title'])) {
            $errors[] = dgettext('vshop', 'You must give this option set a title.');
        } else {
            $this->option_set->setTitle($_POST['title']);
        }

        if (!empty($_POST['type']) ) {
            $this->option_set->setType((int)$_POST['type']);
        }
    ...
    public function postOption_value()
    {
        $this->loadOption_value();
//print_r($_POST); exit;
        if (empty($_POST['title'])) {
            $errors[] = dgettext('vshop', 'You must give this option value a title.');
        } else {
            $this->option_value->setTitle($_POST['title']);
        }

        $this->option_value->setSet_id($_POST['set_id']);
        $this->option_value->setSort($_POST['sort']);
...
//print_r($_POST); exit;
        ...
        if (isset($_POST['set_id'])) {
            $this->attribute->setSet_id($_POST['set_id']);
        } else {
            PHPWS_Core::initModClass('vshop', 'vShop_Option_value.php');
            $value = new vShop_Option_value($_POST['value_id']);
            $this->attribute->setSet_id($value->set_id);
        }
        $this->attribute->setValue_id($_POST['value_id']);
        $this->attribute->setItem_id($_POST['item_id']);
        if (!empty($_POST['price_mod']) ) {
            $this->attribute->setPrice_mod($_POST['price_mod']);
        } else {
            $this->attribute->setPrice_mod(0);
        }
        $this->attribute->setPrice_prefix($_POST['price_prefix']);
        if (!empty($_POST['weight_mod']) ) {
            $this->attribute->setWeight_mod($_POST['weight_mod']);
        } else {
            $this->attribute->setWeight_mod(0);
        }
        $this->attribute->setWeight_prefix($_POST['weight_prefix']);

        if (isset($errors)) {
            $this->message = implode('<br />', $errors);
            $this->loadForm('edit_attribute'); // CHECK !!!
            return false;
        } else {
            return true;
        }
...
print_r($cart_data[$id]['data']->_itemData); //exit; // TEMP
