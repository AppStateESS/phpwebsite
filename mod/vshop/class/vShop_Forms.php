...
//print_r($this->vshop->order); exit;
    ...
    function settingsPanel()
    {
        ...
        if (Current_User::allow('vshop', 'settings')){
            $tags['option_sets'] = array('title'=>dgettext('vshop', 'Option Sets'), 'link'=>$link);
            $tags['option_values'] = array('title'=>dgettext('rolodex', 'Option Values'), 'link'=>$link);
        }
...
//        }
    ...
    public function listOptionSets()
    {
        if (Current_User::allow('vshop', 'settings') && isset($_REQUEST['uop'])) {
            $link[] = PHPWS_Text::secureLink(dgettext('vshop', 'Add new option set'), 'vshop', array('aop'=>'new_option_set'));
            MiniAdmin::add('vshop', $link);
        }

        $ptags['TITLE_HEADER'] = dgettext('vshop', 'Name');
        $ptags['TYPE_HEADER'] = dgettext('vshop', 'Type');
        $ptags['VALUES_HEADER'] = dgettext('vshop', 'Values');

        PHPWS_Core::initModClass('vshop', 'vShop_Option_set.php');
        PHPWS_Core::initCoreClass('DBPager.php');
        $pager = new DBPager('vshop_option_sets', 'vShop_Option_set');
        $pager->setModule('vshop');

        $pager->setOrder('title', 'asc', true);
        $pager->setTemplate('list_option_sets.tpl');
        $pager->addRowTags('rowTag');
        $num = $pager->getTotalRows();
        if ($num == '0') {
            $vars['aop']  = 'menu';
            $vars['tab']  = 'settings';
            $vars2['aop']  = 'new_option_set';
            $ptags['EMPTY_MESSAGE'] = sprintf(dgettext('vshop', 'Check your %s then create a %s to begin'), PHPWS_Text::secureLink(dgettext('vshop', 'Settings'), 'vshop', $vars),  PHPWS_Text::secureLink(dgettext('vshop', 'New Option Set'), 'vshop', $vars2));
        }
        if (Current_User::allow('vshop', 'settings', null, null, true)) {
            $vars['aop']  = 'new_option_set';
            $ptags['ADD_LINK'] = PHPWS_Text::secureLink(dgettext('vshop', 'Add Option Set'), 'vshop', $vars);
        }
        $pager->addPageTags($ptags);
        $pager->addToggle('class="toggle1"');
        $pager->cacheQueries();
        ...
        $this->vshop->content = $pager->get();
        $this->vshop->title = sprintf(dgettext('vshop', '%s Option Sets'), PHPWS_Text::parseOutput(PHPWS_Settings::get('vshop', 'mod_title')));
    }


    public function listOptionValues()
    {

        $ptags['TITLE_HEADER'] = dgettext('vshop', 'Name');
        $ptags['SET_HEADER'] = dgettext('vshop', 'Set');
        $ptags['SORT_HEADER'] = dgettext('vshop', 'Sort');

        PHPWS_Core::initModClass('vshop', 'vShop_Option_value.php');
        PHPWS_Core::initCoreClass('DBPager.php');
        $pager = new DBPager('vshop_option_values', 'vShop_Option_value');
        $pager->setModule('vshop');

        $pager->setOrder('title', 'asc', true);
        $pager->setTemplate('list_option_values.tpl');
        $pager->addRowTags('rowTag');
        $pager->addPageTags($ptags);
        $pager->addToggle('class="toggle1"');
        $pager->cacheQueries();
        ...
        $this->vshop->content = $pager->get();
        $this->vshop->title = sprintf(dgettext('vshop', '%s Option Values'), PHPWS_Text::parseOutput(PHPWS_Settings::get('vshop', 'mod_title')));
    }
...
//        $form->setRequired('weight');
    ...
    //        $form->setRequired('shipping');
        ...
        $tpl['ATTRIBUTES_LABEL'] = dgettext('vshop', 'Item options');
        if ($this->getQtyValues() > 0) {
            $tpl['ADD_ATTRIBUTE_LINK'] = PHPWS_Text::secureLink(dgettext('vshop', 'Add Attribute'), 'vshop', array('aop'=>'new_attribute', 'item_id'=>$item->id));
        } else {
            $tpl['ADD_ATTRIBUTE_LINK'] = dgettext('vshop', 'Sorry, but you must define option sets and values before you may add an attribute to an item.');
        }

        $attributes = $item->getAllAttributes();
        if (PHPWS_Error::logIfError($attributes)) {
            $this->vshop->content = dgettext('vshop', 'An error occurred when accessing this items\'s attributes.');
            return;
        }

        if ($attributes) {
            $tpl['ATTRIBUTE_SET_LABEL'] = dgettext('vshop', 'Set');
            $tpl['ATTRIBUTE_VALUE_LABEL'] = dgettext('vshop', 'Value');
            $tpl['ATTRIBUTE_PRICE_LABEL'] = dgettext('vshop', 'Price mod');
            $tpl['ATTRIBUTE_WEIGHT_LABEL'] = dgettext('vshop', 'Weight mod');
            foreach ($attributes as $attribute) {
                $tpl['attributes'][] = $attribute->viewTpl();
            }
        } else {
    ...
    public function editOptionSet()
    {
        $form = new PHPWS_Form;
        $option_set = & $this->vshop->option_set;
        require PHPWS_SOURCE_DIR . 'mod/vshop/inc/option_types.php';

        $form->addHidden('module', 'vshop');
        $form->addHidden('aop', 'post_option_set');
        if ($option_set->id) {
            $this->vshop->title = dgettext('vshop', 'Update option set');
            $form->addHidden('option_set_id', $option_set->id);
            $form->addSubmit(dgettext('vshop', 'Update'));
        } else {
            $this->vshop->title = dgettext('vshop', 'Add new option set');
            $form->addSubmit(dgettext('vshop', 'Add'));
        }

        $form->addText('title', $option_set->title);
        $form->setSize('title', 40);
        $form->setRequired('title');
        $form->setLabel('title', dgettext('vshop', 'Title'));

        $form->addSelect('type', $types);
        $form->setLabel('type', dgettext('vshop', 'Element type'));
        $form->setMatch('type', $option_set->type);

        $tpl = $form->getTemplate();
        $tpl['INFO_LABEL'] = dgettext('vshop', 'Option set details');

        $this->vshop->content = PHPWS_Template::process($tpl, 'vshop', 'edit_option_set.tpl');
    }
...
//        require PHPWS_SOURCE_DIR . 'mod/vshop/inc/zones.php';
        ...
        if ($option_value->id) {
            $this->vshop->title = dgettext('vshop', 'Update option set value');
            $form->addHidden('option_value_id', $option_value->id);
            $form->addSubmit(dgettext('vshop', 'Update'));
        } else {
            $this->vshop->title = dgettext('vshop', 'Add new option set value');
            $form->addSubmit(dgettext('vshop', 'Add'));
        }

        $form->addText('title', $option_value->title);
        $form->setSize('title', 40);
        $form->setRequired('title');
        $form->setLabel('title', dgettext('vshop', 'Title'));
        ...
        $db->addColumn('id');
        $db->addColumn('title');
        $result = $db->getObjects('vShop_Option_set');
        if ($result) {
            foreach ($result as $set) {
                $choices[$set->id] = $set->title;
            }
            $form->addSelect('set_id', $choices);
            $form->setLabel('set_id', dgettext('vshop', 'Option Set'));
            $form->setMatch('set_id', $option_value->set_id);
        }

        $form->addText('sort', $option_value->sort);
        $form->setSize('sort', 3);
        $form->setRequired('sort');
        $form->setLabel('sort', dgettext('vshop', 'Sort order'));

        $tpl = $form->getTemplate();
        $tpl['INFO_LABEL'] = dgettext('vshop', 'Option set value details');

        $this->vshop->content = PHPWS_Template::process($tpl, 'vshop', 'edit_option_value.tpl');
    }
...
//print_r($item); exit;
        ...
        if ($attribute->id) {
            $this->vshop->title = dgettext('vshop', 'Update attribute');
            $form->addHidden('attribute_id', $attribute->id);
            $form->addHidden('set_id', $attribute->set_id);
            $form->addSubmit(dgettext('vshop', 'Update'));
        } else {
            $this->vshop->title = dgettext('vshop', 'Add new attribute');
            $form->addSubmit(dgettext('vshop', 'Add'));
        }
        $form->addHidden('item_id', $item->id);
        ...
        $db->addColumn('set_id');
        $db->addColumn('title');
        $db->addColumn('vshop_option_sets.title', null, 'setname');
        $db->addOrder('vshop_option_sets.title asc');
...
//        $db->setTestMode();
        ...
        if ($result) {
            foreach ($result as $value) {
                $name = $value['setname'] . ' : ' . $value['title'];
                $choices[$value['id']] = PHPWS_Text::parseOutput($name);
            }
            $form->addSelect('value_id', $choices);
            $form->setLabel('value_id', dgettext('vshop', 'Option value'));
            $form->setMatch('value_id', $attribute->value_id);
        }

        $choices = array('+'=>'+','-'=>'-');
        $form->addSelect('price_prefix', $choices);
        $form->setLabel('price_prefix', dgettext('vshop', 'Price prefix'));
        $form->setMatch('price_prefix', $attribute->price_prefix);
...
//        $form->setRequired('price_mod');
        ...
        $choices = array('+'=>'+','-'=>'-');
        $form->addSelect('weight_prefix', $choices);
        $form->setLabel('weight_prefix', dgettext('vshop', 'Weight prefix'));
        $form->setMatch('weight_prefix', $attribute->weight_prefix);
...
//        $form->setRequired('weight_mod');
        ...
        $tpl = $form->getTemplate();
        $tpl['INFO_LABEL'] = dgettext('vshop', 'Attribute details');

        $this->vshop->content = PHPWS_Template::process($tpl, 'vshop', 'edit_attribute.tpl');
    }
