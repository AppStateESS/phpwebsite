...
<table class="box phpwsbb" cellspacing="1" cellpadding="4">
    <tr class="box-title">
        ...
        <th width="10%" valign="middle" style="text-align: center"
            scope="col"
        >{TOPICS_TITLE}</th>
    ...
    <!-- BEGIN CAT_ROW -->
    <tr>
        <td colspan="5" class="phpwsbb_forum_category_header"><!-- BEGIN CATEGORY_IMAGE -->
        <div style="float: left; padding-right: 10px">
        {CATEGORY_ICON}</div>
        <!-- END CATEGORY_IMAGE -->
        <h2>{SECTION_TITLE} : {CATEGORY_NAME}</h2>
        </td>
    </tr>
    <!-- END CAT_ROW -->
    ...
    <tr>
        ...
        <td style="vertical-align: middle"><span
            style="float: right;"
        > {FORUM_LOCKED} </span> <span style="white-space: nowrap">{FORUM_TITLE_LINK}</span><br />
        <span class="smaller">{FORUM_DESCRIPTION}</span></td>
        ...
        >{FORUM_LASTPOST_POST_LINK} {FORUM_LASTPOST_DATE_SHORT}<br
            style="clear: left"
        />
        {IN} {FORUM_LASTPOST_TOPIC_LINK}<br />
        {BY} {FORUM_LASTPOST_AUTHOR}</td>
    </tr>
    <!-- END cat_forum_list -->
    <!-- END cat_list -->
</table>
