...
class com.jeroenwijering.utils.StringMagic {
	...
	**/
	static function toSeconds(str:String):Number {
		var arr = str.split(':');
		if (str.substr(-1) == 's') {
			var sec = Number(str.substr(0,str.length-2));
		} else if (str.substr(-1) == 'm') {
			var sec = Number(str.substr(0,str.length-2))*60;
		} else if(str.substr(-1) == 'h') {
			var sec = Number(str.substr(0,str.length-2))*3600;
		} else if(arr.length > 1) {
			var sec = Number(arr[arr.length-1]);
