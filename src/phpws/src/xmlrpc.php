...
class MyServer extends IXR_IntrospectionServer {
    ...
    public function __construct()
    {
        ...
        $this->addCallback(
                'test.isLoggedIn', 'this:isLoggedIn',
                array('string', 'string', 'string'), 'Returns user status'
        );
        $this->addCallback(
                'test.getDate', 'this:getDate', array('string'),
                'Returns the current date'
        );
        $this->addCallback(
                'test.getTime', 'this:getTime', array('string'),
                'Returns the current time'
        );
        $this->addCallback(
                'test.helloWorld', 'this:helloWorld', array('string'),
                'Returns "Hello World"'
        );
        $this->addCallback(
                'test.error', 'this:ooh', array('struct'),
                'Triggers an error response'
        );
        $this->addCallback(
                'test.multiplied', 'this:times10', array('int', 'struct'),
    ...
    public function metaWeblog_editPost($args)
    {
        ...
        if (empty($args[3])) {
            return new IXR_Error(5010,
                    'Missing reference key. Unpublished material cannot be categorized.');
        }
    ...
    {
        \phpws\PHPWS_Core::requireInc('core', 'FileTypes.php');
        ...
        if (!(ALLOW_OCTET_STREAM && $filetype == 'application/octet-stream') && !in_array($filetype,
                        $allowed_images)) {
...
function record($file, $info, $append = true)
{
    ...
    if ($append) {
        file_put_contents(LOG_DIR . $file . '.txt',
                strftime('%T') . ' ' . $info . "\n", FILE_APPEND);
    } else {
        file_put_contents(LOG_DIR . $file . '.txt',
                strftime('%T') . ' ' . $info . "\n");
