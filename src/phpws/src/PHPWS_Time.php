...
class PHPWS_Time {
	...
	public static function convertUserTime($time)
	{
	...
	public static function getServerTime($time=0)
	{
		if (!$time) {
			$time = time();
		}
	...
	public static function getUserTime($time=0)
	{
		if (!$time) {
			$time = time();
		}
	...
	{
		$file = \phpws\PHPWS_Core::getConfigFile('core', 'Time.php');
		include $file;
		return $timezones;
	}
